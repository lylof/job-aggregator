...
## 🚨 TÂCHES CRITIQUES DE REDRESSEMENT (Ajoutées suite à l'audit technique)

- [ ] 16. Phase 0 - Stabilisation Fondamentale (PRIORITÉ ABSOLUE)
  - [ ] 16.1 Résolution des imports relatifs


    - Remplacer tous les imports `from ...utils` par des imports absolus `from jinascraper.utils`
    - Corriger les 6 fichiers de nettoyeurs d'URL identifiés
    - Supprimer le dossier parasite `jinascraper/jinascraper/`
    - Supprimer le fichier obsolète `sources_config.py`
    - _Criticité: BLOQUANT - Aucun test ne fonctionne sans cette correction_

  - [ ] 16.2 Validation de la base de code
    - Créer `test_imports_validation.py` pour vérifier tous les imports
    - Valider que tous les modules s'importent sans erreur
    - Éliminer tous les bricolages `sys.path.insert()` dans les tests
    - _Criticité: BLOQUANT - Base de code non-testable actuellement_

- [ ] 17. Phase 1 - Réparation des Sources Critiques (PRIORITÉ HAUTE)
  - [ ] 17.1 Réparation EmploiTogo.info (0 URLs → 15 URLs attendues)
    - Supprimer `/emploitogo/` des `excluded_paths` dans le nettoyeur
    - Ajouter le pattern `r'^/emploitogo/offre-demploi-[^/]+/?$'` aux `valid_patterns`
    - Tester avec les URLs réelles extraites par Jina
    - _Criticité: CRITIQUE - Source complètement non-fonctionnelle_

  - [ ] 17.2 Correction YOP L-FRII (1 URL → 15+ URLs attendues)
    - Analyser la structure HTML réelle de `yop.l-frii.com/offres-demplois/`
    - Corriger le sélecteur CSS `h2.elementor-heading-title a` qui génère des doublons
    - Identifier le sélecteur qui capture chaque offre une seule fois
    - Valider avec Jina Reader pour éliminer les doublons
    - _Criticité: HAUTE - Source dégradée avec extraction insuffisante_

  - [ ] 17.3 Tests de régression pour sources fonctionnelles
    - Créer des tests automatisés pour LinkedIn Togo (40 URLs)
    - Créer des tests automatisés pour ANPE Togo (15 URLs)  
    - Créer des tests automatisés pour Emploi.tg (25 URLs)
    - Établir des seuils d'alerte en cas de régression
    - _Criticité: MOYENNE - Prévention des régressions futures_

- [ ] 18. Phase 2 - Harnais de Test Robuste (PRIORITÉ MOYENNE)
  - [ ] 18.1 Tests de surveillance continue
    - Créer `test_source_monitoring.py` avec seuils par source
    - Implémenter la détection automatique des régressions
    - Ajouter des métriques de qualité (URLs extraites, taux de nettoyage)
    - _Criticité: MOYENNE - Prévention des problèmes futurs_

  - [ ] 18.2 Validation architecturale
    - Créer des tests pour valider la cohérence des imports
    - Implémenter des tests de structure de package
    - Ajouter des tests de conformité aux steering files
    - _Criticité: MOYENNE - Maintien de la qualité architecturale_
...
### Critères de Succès Post-Audit (24/07/2025)
- **Architecture** : 0 ImportError autorisé dans tout le système
- **Sources Critiques** : EmploiTogo.info doit extraire ≥10 URLs (actuellement 0)
- **Sources Dégradées** : YOP L-FRII doit extraire ≥15 URLs uniques (actuellement 1)
- **Sources Fonctionnelles** : Maintenir LinkedIn (40), ANPE (15), Emploi.tg (25)
- **Qualité Code** : 100% conformité aux nouvelles règles d'imports
- **Tests** : Couverture ≥90% pour tous les nettoyeurs d'URL